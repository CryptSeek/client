...
async function handleMessageReceived(event, message) {
    ...
    if (identifier === keyPair["ident"]) {
        ...
        fs.writeFileSync(statusFile, JSON.stringify({status: 1}));
    ...
    try {
        friendStatus = JSON.parse(fs.readFileSync(statusFile).toString())['status'];
    } catch (err) {
        friendStatus = 0;
    }
    ...
    if (friendStatus === 3 || friendStatus === 1 || friendStatus === 0 || parsed['sender'] === store.get('username')) {
        ...
        if (friendStatus === 1) {
            fs.writeFileSync(statusFile, JSON.stringify({status: 2}));
        }
    }
